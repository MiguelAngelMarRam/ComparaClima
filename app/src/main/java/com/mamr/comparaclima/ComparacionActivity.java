...
public class ComparacionActivity extends AppCompatActivity {
    ...
    private boolean pred1rx = false, pred2rx = false;

    private DatabaseReference mDatabase;
    private String userId;

    private ArrayAdapter<String> adapterDestinos;
    private List<String> listaDestinosStrings = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isNetworkAvailable()) {
            Toast.makeText(this, "Sin conexión: los gustos y comparaciones pueden no estar actualizados.", Toast.LENGTH_LONG).show();
        }

        listaDestinosStrings.add("Solo Clima");
        adapterDestinos = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, listaDestinosStrings);
        ...
        btnComparar.setOnClickListener(v -> {
            // COMPROBACIÓN DE RED ANTES DE EMPEZAR
            if (!isNetworkAvailable()) {
                Toast.makeText(this, "No hay conexión a internet para consultar el clima", Toast.LENGTH_LONG).show();
                return;
            }
    ...
    private void cargarMunicipiosDesdeAssets() {
        try {
            InputStream is = getAssets().open("municipios.json");
            ...
            Gson gson = new Gson();
            Type listType = new TypeToken<List<Municipio>>(){}.getType();
            List<Municipio> localList = gson.fromJson(json, listType);

            if (localList != null && !localList.isEmpty()) {
                listaMunicipios = localList;
                actualizarAdaptadores();
                Log.d("LOCAL_DATA", "Municipios cargados exitosamente desde Assets");
            }
        } catch (IOException e) {
            Log.e("LOCAL_DATA", "Error leyendo municipios.json: " + e.getMessage());
            Toast.makeText(this, "Error al cargar base de datos local de municipios", Toast.LENGTH_SHORT).show();
        }
    }
        ...
        api.getClimaFinal(urlReal).enqueue(new Callback<List<PrediccionRespuesta>>() {
            ...
            public void onResponse(Call<List<PrediccionRespuesta>> call, Response<List<PrediccionRespuesta>> response) {
                if (response.isSuccessful() && response.body() != null && !response.body().isEmpty()) {
                    ...
                    Toast.makeText(ComparacionActivity.this, "AEMET no pudo devolver el detalle del clima", Toast.LENGTH_SHORT).show();
                }
            }
            @Override public void onFailure(Call<List<PrediccionRespuesta>> call, Throwable t) {
                btnComparar.setEnabled(true);
                Toast.makeText(ComparacionActivity.this, "Error de red al obtener detalle", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void comprobarYProcesar() {
        if (pred1rx && pred2rx) {
            ...
            String modoSel = spinnerDestino.getSelectedItem().toString();

            guardarEnHistorialFirebase(auto1.getText().toString(), auto2.getText().toString(), modoSel);

            Intent intent = new Intent(this, ResultadoActivity.class);
            intent.putExtra("DATA_C1", (Serializable) datosCiudad1);
            intent.putExtra("DATA_C2", (Serializable) datosCiudad2);
            intent.putExtra("TIPO_DESTINO", modoSel);
    ...
    private void guardarEnHistorialFirebase(String c1, String c2, String modo) {
        if (userId != null && isNetworkAvailable()) {
            DatabaseReference historialRef = mDatabase.child("users").child(userId).child("historial");
            String key = historialRef.push().getKey();
            SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm", Locale.getDefault());
            String fechaActual = sdf.format(Calendar.getInstance().getTime());
            Map<String, Object> registro = new HashMap<>();
            registro.put("ciudad1", c1);
            registro.put("ciudad2", c2);
            registro.put("modo", modo);
            registro.put("fecha", fechaActual);
            if (key != null) historialRef.child(key).setValue(registro);
        }
    }

    private void configurarSpinnerDestinos() {
        if (userId == null) return;
        mDatabase.child("users").child(userId).child("presets").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                listaDestinosStrings.clear();
                listaDestinosStrings.add("Solo Clima");
                for (DataSnapshot ds : snapshot.getChildren()) {
                    listaDestinosStrings.add(ds.getKey());
                }
                adapterDestinos.notifyDataSetChanged();
            }
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(ComparacionActivity.this, "No se pudieron sincronizar tus destinos de la nube", Toast.LENGTH_SHORT).show();
            }
        });
    }
        ...
        api.getUrlPrediccion(id, MI_API_KEY).enqueue(new Callback<AemetRespuesta>() {
            ...
            public void onResponse(Call<AemetRespuesta> call, Response<AemetRespuesta> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    btnComparar.setEnabled(true);
                    Toast.makeText(ComparacionActivity.this, "AEMET no responde (Código " + response.code() + ")", Toast.LENGTH_SHORT).show();
                }
            }
            @Override
            public void onFailure(Call<AemetRespuesta> call, Throwable t) {
                btnComparar.setEnabled(true);
                Toast.makeText(ComparacionActivity.this, "Error crítico de conexión", Toast.LENGTH_LONG).show();
            }
        });
    }
            ...
            return new Filter() {
                @Override protected FilterResults performFiltering(CharSequence constraint) {
                    ...
                    if (constraint != null && constraint.length() > 0) {
                        ...
                        List<Municipio> suggestions = new ArrayList<>();
                        for (Municipio m : fullList) {
                            if (normalizar(m.getNombre()).contains(filterPattern)) suggestions.add(m);
                        }
